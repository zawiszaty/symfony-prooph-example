...
Zawsze jest metoda zatwierdzająca jednak różnią sie one sposobem zatwierdzania. Ja dodaje do swojego aggregatu zawsze taki kod. Pozwala on mi na definiowanie metod w takiej konwencji. Jeżeli aggregat zanotował event AuthorWasCreated metoda zatwierdzająca to applyAuthorWasCreated(AuthorWasCreated $event)
```php
    /**
     * Apply given event.
     */
